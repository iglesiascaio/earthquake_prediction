...
def undersample_by_matching(X_train, y_train, feats):
    """
    ...
    sel_abs = neg_idx[sel]
    keep = np.concatenate([pos_idx, sel_abs])

    print(
        f"Matching → retained {len(keep)} samples "
        f"({len(pos_idx)} pos + {len(sel_abs)} neg)"
    )
    return X_train.iloc[keep].reset_index(drop=True), y_train.iloc[keep].reset_index(
        drop=True
    )
...
# 5. Main run
# ----------------------------------------------------------
undersampling_random = False  # ⇠ NEW switch
...
if undersampling_random:
    ...
    X_train, y_train = undersample_by_matching(X_train, y_train, feats)
